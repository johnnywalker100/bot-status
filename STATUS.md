# Paper-Trading Status

*Auto-updated by each hourly tick. Equity is a multiplier of starting value
(simulated — no real money). All returns net of taker fees, tiered slippage,
and 8h funding settlements.*

![equity](status_equity.png)

| | |
|---|---|
| **Equity** | **1.0204** (+2.04% since start) |
| Peak / drawdown | 1.0572 / -3.49% |
| Ticks recorded | 401 |
| Last tick | 2026-08-09T06:08:07.487214+00:00 (+0.1322%) |
| Risk rails | normal (dd -3.5%); blind 2.1h -> 50% size on resume |
| Data source | okx (bar 2026-08-09 05:00:00+00:00) |
| Gross leverage | 1.67x |
| Weeks tracked | 2 |
| Average week | +1.10% |
| Weeks ≥ +3% | 50% |
| Best / worst week | +3.03% / -0.83% |

## Positions (futures contracts, fraction of account)

**Long perps** | **Short perps**

| Contract | Size |
|---|---|
| TRX perp | +13.0% |
| XLM perp | +9.8% |
| ETH perp | +8.8% |
| THETA perp | +7.9% |
| AAVE perp | +7.0% |
| XRP perp | +5.9% |
| LTC perp | +3.1% |
| ETC perp | +2.2% |
| ADA perp | +2.1% |
| SOL perp | +1.9% |
| AVAX perp | +0.5% |

| Contract | Size |
|---|---|
| DOGE perp | -16.9% |
| UNI perp | -16.1% |
| SUSHI perp | -14.9% |
| BCH perp | -13.2% |
| BNB perp | -9.8% |
| LINK perp | -9.4% |
| FIL perp | -8.8% |
| ALGO perp | -6.1% |
| DOT perp | -4.1% |
| BTC perp | -3.8% |
| ATOM perp | -2.0% |

*Longs collect when price rises; shorts collect when price falls. The book is
mostly market-neutral: it earns funding spread + momentum, not a bet that
crypto goes up.*
