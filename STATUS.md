# Paper-Trading Status

*Auto-updated by each hourly tick. Equity is a multiplier of starting value
(simulated — no real money). All returns net of taker fees, tiered slippage,
and 8h funding settlements.*

![equity](status_equity.png)

| | |
|---|---|
| **Equity** | **1.0347** (+3.47% since start) |
| Peak / drawdown | 1.0572 / -2.13% |
| Ticks recorded | 245 |
| Last tick | 2026-08-02T15:08:01.616295+00:00 (-0.0973%) |
| Risk rails | normal (dd -2.1%); blind 2.1h -> 50% size on resume |
| Data source | okx (bar 2026-08-02 14:00:00+00:00) |
| Gross leverage | 1.56x |
| Weeks tracked | 1 |
| Average week | +3.62% |
| Weeks ≥ +3% | 100% |
| Best / worst week | +3.62% / +3.62% |

## Positions (futures contracts, fraction of account)

**Long perps** | **Short perps**

| Contract | Size |
|---|---|
| XLM perp | +11.9% |
| TRX perp | +10.4% |
| ETH perp | +7.0% |
| DOT perp | +6.6% |
| ETC perp | +6.0% |
| ALGO perp | +3.3% |
| AVAX perp | +1.6% |
| SOL perp | +0.9% |

| Contract | Size |
|---|---|
| BNB perp | -22.5% |
| SUSHI perp | -14.7% |
| DOGE perp | -12.9% |
| LINK perp | -12.1% |
| LTC perp | -10.6% |
| BCH perp | -7.2% |
| BTC perp | -7.2% |
| THETA perp | -6.3% |
| UNI perp | -5.8% |
| FIL perp | -3.3% |
| ADA perp | -2.2% |
| ATOM perp | -2.1% |
| AAVE perp | -1.5% |

*Longs collect when price rises; shorts collect when price falls. The book is
mostly market-neutral: it earns funding spread + momentum, not a bet that
crypto goes up.*
