# Paper-Trading Status

*Auto-updated by each hourly tick. Equity is a multiplier of starting value
(simulated — no real money). All returns net of taker fees, tiered slippage,
and 8h funding settlements.*

![equity](status_equity.png)

| | |
|---|---|
| **Equity** | **0.9702** (-2.98% since start) |
| Peak / drawdown | 1.0572 / -8.23% |
| Ticks recorded | 739 |
| Last tick | 2026-08-23T11:07:53.657425+00:00 (+0.1608%) |
| Risk rails | normal (dd -8.2%); blind 2.1h -> 50% size on resume |
| Data source | okx (bar 2026-08-23 10:00:00+00:00) |
| Gross leverage | 0.45x |
| Weeks tracked | 4 |
| Average week | -0.67% |
| Weeks ≥ +3% | 25% |
| Best / worst week | +3.03% / -5.48% |

## Positions (futures contracts, fraction of account)

**Long perps** | **Short perps**

| Contract | Size |
|---|---|
| ALGO perp | +4.8% |
| TRX perp | +4.7% |
| XLM perp | +4.0% |
| AAVE perp | +3.6% |
| BTC perp | +3.2% |
| ATOM perp | +3.0% |
| BCH perp | +2.8% |
| LINK perp | +2.5% |
| DOT perp | +2.5% |
| ETC perp | +2.4% |
| THETA perp | +1.9% |
| BNB perp | +1.5% |
| ADA perp | +1.4% |
| SOL perp | +1.2% |
| ETH perp | +1.0% |
| SUSHI perp | +0.9% |
| XRP perp | +0.6% |

| Contract | Size |
|---|---|
| UNI perp | -2.2% |
| AVAX perp | -1.0% |

*Longs collect when price rises; shorts collect when price falls. The book is
mostly market-neutral: it earns funding spread + momentum, not a bet that
crypto goes up.*
